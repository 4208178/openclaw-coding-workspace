...
2. **本地备份** (`docs/skills/local-backup/`)
   - 自动备份工作区，防止数据丢失。
   - 触发：`./docs/skills/local-backup/run.sh`
3. **技术债务追踪器** (计划中)
...
### 🧠 吸收外部技能
- **elite-longterm-memory**: 已对齐当前记忆系统，增强维护逻辑。
- **evolution-engine**: 已对齐当前进化策略，优化反馈闭环。
- **xiucheng-self-improving-agent**: 吸收“自我诊断”逻辑，集成到代码审查助手。
